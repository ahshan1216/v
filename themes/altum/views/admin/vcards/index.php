...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 m-0"><i class="fa fa-fw fa-xs fa-id-card text-primary-900 mr-2"></i> <?= l('admin_vcards.header') ?></h1>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/vcards?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= l('global.export_csv') ?>
                    </a>
                    <a href="<?= url('admin/vcards?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/vcards') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_vcards.table.name') ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_vcards.table.name') ?></option>
                                <option value="pageviews" <?= $data->filters->order_by == 'pageviews' ? 'selected="selected"' : null ?>><?= l('admin_vcards.table.pageviews') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/vcards/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('admin_vcards.table.user') ?></th>
            <th><?= l('admin_vcards.table.name') ?></th>
            <th><?= l('admin_vcards.table.pageviews') ?></th>
            <th><?= l('admin_vcards.table.is_enabled') ?></th>
            <th></th>
            <th><?= l('admin_vcards.table.datetime') ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        <?php foreach($data->vcards as $row): ?>
            <tr>
                ...
                <td class="text-nowrap">
                    <?= $row->name ?><a href="<?= url('vcard-redirect/' . $row->vcard_id) ?>" rel="noreferrer" target="_blank"><i class="fa fa-fw fa-xs fa-external-link-alt ml-1"></i></a>
                </td>
                <td class="text-nowrap">
                    <div class="d-flex flex-column">
                ...
                <td class="text-nowrap">
                    <a href="<?= url('admin/vcards-blocks?vcard_id=' . $row->vcard_id) ?>" data-toggle="tooltip" title="<?= l('admin_vcards.table.vcard_blocks') ?>">
                        <i class="fa fa-fw fa-bars text-muted"></i>
                    </a>
                </td>
                <td class="text-nowrap">
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/vcards/admin_vcard_dropdown_button.php', ['id' => $row->vcard_id]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/admin/vcards/vcard_delete_modal.php'), 'modals'); ?>
