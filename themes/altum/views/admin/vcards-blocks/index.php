...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 m-0"><i class="fa fa-fw fa-xs fa-id-card text-primary-900 mr-2"></i> <?= l('admin_vcards_blocks.header') ?></h1>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/vcards-blocks?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= l('global.export_csv') ?>
                    </a>
                    <a href="<?= url('admin/vcards-blocks?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/vcards-blocks') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_vcards_blocks.table.name') ?></option>
                                <option value="value" <?= $data->filters->search_by == 'value' ? 'selected="selected"' : null ?>><?= l('admin_vcards_blocks.table.value') ?></option>
                            </select>
                        </div>

                        <div class="form-group px-4">
                            <label for="filters_type" class="small"><?= l('admin_vcards_blocks.table.type') ?></label>
                            <select name="type" id="filters_type" class="form-control form-control-sm">
                                ...
                                <?php foreach(require APP_PATH . 'includes/v/vcards_blocks.php' as $key => $value): ?>
                                    <option value="<?= $key ?>" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == $key ? 'selected="selected"' : null ?>><?= l('vcards_blocks.' . $key) ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="pageviews" <?= $data->filters->order_by == 'pageviews' ? 'selected="selected"' : null ?>><?= l('admin_vcards_blocks.table.pageviews') ?></option>
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_vcards_blocks.table.name') ?></option>
                                <option value="value" <?= $data->filters->order_by == 'value' ? 'selected="selected"' : null ?>><?= l('admin_vcards_blocks.table.value') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/vcards-blocks/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('admin_vcards_blocks.table.user') ?></th>
            <th><?= l('admin_vcards_blocks.table.type') ?></th>
            <th><?= l('admin_vcards_blocks.table.name') ?></th>
            <th><?= l('admin_vcards_blocks.table.pageviews') ?></th>
            <th><?= l('admin_vcards_blocks.table.is_enabled') ?></th>
            <th><?= l('admin_vcards_blocks.table.datetime') ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        <?php foreach($data->vcards_blocks as $row): ?>
            <tr>
                <td data-bulk-table class="d-none">
                    <div class="custom-control custom-checkbox">
                        <input id="selected_vcard_id_<?= $row->vcard_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->vcard_id ?>" />
                        <label class="custom-control-label" for="selected_vcard_id_<?= $row->vcard_id ?>"></label>
                    </div>
                </td>
                ...
                <td class="text-nowrap">
                    <?= l('vcards_blocks.' . $row->type) ?>
                </td>
                <td class="text-nowrap">
                    <div class="d-flex flex-column">
                        <div>
                            <span class="text-truncate"><?= $row->name ?></span> <a href="<?= url('vcard-redirect/' . $row->vcard_id) ?>" rel="noreferrer" target="_blank"><i class="fa fa-fw fa-xs fa-external-link-alt ml-1"></i></a>
                        </div>
                        <span class="text-muted text-truncate"><?= $row->value ?></span>
                    </div>
                </td>
                <td class="text-nowrap">
                    <div class="d-flex flex-column">
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/vcards-blocks/admin_vcard_block_dropdown_button.php', ['id' => $row->vcard_block_id]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/admin/vcards-blocks/vcard_block_delete_modal.php'), 'modals'); ?>
