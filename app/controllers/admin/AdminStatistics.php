...
class AdminStatistics extends Controller {
    ...
    protected function vcards_blocks() {

        $total = ['vcards_blocks' => 0];
        ...
        while($row = $result->fetch_object()) {
            ...
            $vcards_blocks_chart[$row->formatted_date] = [
                'vcards_blocks' => $row->total
            ];

            $total['vcards_blocks'] += $row->total;
        }

        $vcards_blocks_chart = get_chart_data($vcards_blocks_chart);

        return [
            'total' => $total,
            'vcards_blocks_chart' => $vcards_blocks_chart,
        ];
