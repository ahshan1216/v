...
class Router {
    ...
    public static $routes = [
        ...
        'admin' => [
            'vcards' => [
                'controller' => 'AdminVcards',
            ],

            'vcards-blocks' => [
                'controller' => 'AdminVcardsBlocks',
            ],
    ...
    public static function parse_controller() {
        ...
        if($original_url_host != $request_url_host) {
            ...
            if($domain && $domain->is_enabled) {
                ...
                /* Check for a unique page assigned to the domain */
                if(empty(self::$params[0]) && self::$data['domain']->vcard_id) {
                    $vcard = (new \Altum\Models\Vcard())->get_vcard_by_vcard_id(Router::$data['domain']->vcard_id);

                    if($vcard) {
                        $vcard->full_url = Router::$data['domain']->scheme . Router::$data['domain']->host . '/';

                        /* Set some route data */
                        self::$data['vcard'] = $vcard;
                    }
                }

                /* Check for a custom domain index redirect */
                else if(empty(self::$params[0]) && self::$data['domain']->custom_index_url) {
        ...
        /* Check for potential vcard link */
        if(self::$path == 'v') {
            /* Vcard page */
            self::$controller_key = 'vcard';
            self::$controller = 'Vcard';
        }
        ...
        if(!empty(self::$params[0])) {
            ...
            if(array_key_exists(self::$params[0], self::$routes[self::$path]) && file_exists(APP_PATH . 'controllers/' . (self::$path != '' ? self::$path . '/' : null) . self::$routes[self::$path][self::$params[0]]['controller'] . '.php')) {
                ...
                $cache_instance = \Altum\Cache::$adapter->getItem('v_vcard?url=' . md5(self::$params[0]) . (isset(self::$data['domain']) ? '&domain_id=' . self::$data['domain']->domain_id : null));
                ...
                if(!$cache_instance->get()) {
                    ...
                    if(isset(self::$data['domain'])) {
                        $vcard = db()->where('url', self::$params[0])->where('domain_id', self::$data['domain']->domain_id)->getOne('vcards');
                        if($vcard) $vcard->full_url = self::$data['domain']->scheme . self::$data['domain']->host . '/' . self::$params[0] . '/';
                    } else {
                        $vcard = db()->where('url', self::$params[0])->where('domain_id', NULL, 'IS')->getOne('vcards');
                        if($vcard) $vcard->full_url = SITE_URL . self::$params[0] . '/';
                    }

                    /* Save cache */
                    if($vcard) {
                        \Altum\Cache::$adapter->save($cache_instance->set($vcard)->expiresAfter(CACHE_DEFAULT_SECONDS)->addTag('vcard_id=' . $vcard->vcard_id));
