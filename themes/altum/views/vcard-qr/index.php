...
<div class="container">
    ...
    <nav aria-label="breadcrumb">
        <ol class="custom-breadcrumbs small">
            ...
            <li>
                <?= l('vcard.breadcrumb') ?><i class="fa fa-fw fa-angle-right"></i>
            </li>
            <li class="active" aria-current="page"><?= l('vcard_qr.breadcrumb') ?></li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0"><?= sprintf(l('vcard_qr.header'), $data->vcard->name) ?></h1>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="foreground_color"><?= l('vcard_qr.configurator.foreground_color') ?></label>
                        <input type="color" id="foreground_color" name="foreground_color" class="form-control" value="#000000" />
                    </div>

                    <div class="form-group">
                        <label for="background_color"><?= l('vcard_qr.configurator.background_color') ?></label>
                        <input type="color" id="background_color" name="background_color" class="form-control" value="#ffffff" />
                    </div>

                    <div class="form-group">
                        <label for="corner_radius"><?= l('vcard_qr.configurator.corner_radius') ?></label>
                        <input type="range" id="corner_radius" name="corner_radius" class="form-control-range" min="0" max="0.5" step="0.1" value="0" />
                    </div>

                    <div class="form-group">
                        <label for="type"><?= l('vcard_qr.configurator.type') ?></label>
                        <select name="type" id="type" class="form-control">
                            <option value="normal" selected="selected"><?= l('vcard_qr.configurator.type_normal') ?></option>
                            <option value="text"><?= l('vcard_qr.configurator.type_text') ?></option>
                            <option value="image"><?= l('vcard_qr.configurator.type_image') ?></option>
                        </select>
                    </div>

                    <div id="type_text" class="d-none">
                        <div class="form-group">
                            <label for="text"><?= l('vcard_qr.configurator.text') ?></label>
                            <input type="text" id="text" name="text" class="form-control" value="" />
                        </div>

                        <div class="form-group">
                            <label for="text_color"><?= l('vcard_qr.configurator.text_color') ?></label>
                            <input type="color" id="text_color" name="text_color" class="form-control" value="#000000" />
                        </div>

                        <div class="form-group">
                            <label for="text_size"><?= l('vcard_qr.configurator.text_size') ?></label>
                    ...
                    <div id="type_image" class="d-none">
                        <div class="form-group">
                            <label for="image"><?= l('vcard_qr.configurator.image') ?></label>
                            <input id="image" type="file" name="image" accept=".png, .jpg, .jpeg" class="form-control-file" />
                            <img id="image-buffer" src="" class="d-none" />
                        </div>

                        <div class="form-group">
                            <label for="image_size"><?= l('vcard_qr.configurator.image_size') ?></label>
                            <input type="range" id="image_size" class="form-control-range" min="0.05" max="0.2" step="0.005" value="0.005" />
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="ecc"><?= l('vcard_qr.configurator.ecc') ?></label>
                        <select name="ecc" id="ecc" class="form-control">
                            <option value="L" selected="selected"><?= l('vcard_qr.configurator.ecc_l') ?></option>
                            <option value="M"><?= l('vcard_qr.configurator.ecc_m') ?></option>
                            <option value="Q"><?= l('vcard_qr.configurator.ecc_q') ?></option>
                            <option value="H"><?= l('vcard_qr.configurator.ecc_h') ?></option>
                        </select>
                    </div>
            ...
            <div class="row mb-4">
                <div class="col-12 col-lg-6 mb-3 mb-lg-0">
                    <button type="button" onclick="window.print()" class="btn btn-block btn-outline-secondary d-print-none">
                        <i class="fa fa-fw fa-sm fa-file-pdf"></i> <?= l('vcard_qr.print') ?>
                    </button>
                </div>

                <div class="col-12 col-lg-6">
                    <button id="download" type="button" class="btn btn-block btn-primary d-print-none">
                        <i class="fa fa-fw fa-sm fa-download"></i> <?= l('vcard_qr.download') ?>
                    </button>
                </div>
            </div>

            <p class="text-muted text-center"><?= l('vcard_qr.subheader') ?></p>
        </div>
    </div>
...
<?php ob_start() ?>
    ...
    <script>
        ...
        let generate_qr = () => {
            let qr_url = <?= json_encode($data->vcard->full_url . '?referrer=qr') ?>;
